...
package player.view.effects.audio;
...
import player.view.SliderControl;
...
public class EqualizerPanel extends BasePanel implements ChangeListener, ItemListener, ActionListener {
	...
	public EqualizerPanel() {
		...
		preampControl = new SliderControl("Preamp", (int) LibVlcConst.MIN_GAIN, (int) LibVlcConst.MAX_GAIN, 0,
				dbFormat);
		...
		for (int i = 0; i < list.size(); i++) {
			bandControls[i] = new SliderControl(formatFrequency(list.get(i)), (int) LibVlcConst.MIN_GAIN,
					(int) LibVlcConst.MAX_GAIN, 0, dbFormat);
