...
public final class NativeLogFrame extends BaseFrame implements LogEventListener {
	...
	public NativeLogFrame(NativeLog nativeLog) {
		...
		this.eventTableModel = GlazedListsSwing.eventTableModelWithThreadProxyList(eventList,
				new NativeLogTableFormat());

		// FIXME Resource
		clearAction = new AbstractAction("Clear") {
			@Override
			public void actionPerformed(ActionEvent e) {
				eventList.clear(); // FIXME this definitely glitched once when
									// messing about with verbosity combo then
									// clearing (some items were left in table
									// until table resized)
			}
		};
	...
	@Override
	public void log(final libvlc_log_level_e level, final String module, final String file, final Integer line,
			final String name, final String header, final Integer id, final String message) {
	...
	private void applyPreferences() {
		...
		setBounds(prefs.getInt("frameX", 200), prefs.getInt("frameY", 200), prefs.getInt("frameWidth", 600),
